...
│   │   └── __init__.py
│   ├── agents/
│   │   ├── llm_client.py       # LLMClient + InputParser + CharacterAgent（三步生成）
...
> 标注 `[llm_trigger]` 的 Storylet 需要玩家输入语义匹配才能进入候选池（已在 `storylet.py._check_llm_trigger()` 实现）。
...
```python
# 三种推进条件（任一满足即推进到 next_landmark）
1. world_state 条件:  指定 flag 全部为 True
2. or_turn_limit:     回合数达到上限（兜底防卡死）
3. or_player_input:   玩家输入匹配关键词或 LLM 语义（当前数据中未使用）
```
...
```python
{
    ...
    "llm_trigger": str | None,        # LLM 语义触发描述（已实现：storylet.py._check_llm_trigger()）
    "content": {
...
1. Beat 模式？──是──→ BeatManager.execute_next()
    │ 否
    ▼
2. InputParser.parse()   （LLM 解析意图/情绪/话题）
    │
    ▼
...
所有 Storylet
    ...
    │              + llm_trigger 语义匹配 ← 已实现（storylet.py._check_llm_trigger()）
    │
...
### 7.6 llm_trigger 语义匹配 (`storylet.py._check_llm_trigger()`) — 已实现

```python
# 已实现的语义触发检测流程：
prompt = f"触发条件：{trigger_condition}\n玩家输入：'{player_input}'\n只回答 YES 或 NO"
response = llm_client.call_llm(prompt, max_tokens=5, temperature=0.0)
# 无 LLM 客户端时保守放行（避免 llm_trigger Storylet 永远沉默）
```

使用 `llm_trigger` 的 Storylet（4个）：
- `sl_father_work_excuse`：玩家问工作/电话
- `sl_player_notices_something_wrong`：玩家察觉异常
- `sl_son_pushes_father`：玩家逼问 Trip
- `sl_father_apology`：玩家劝 Trip 道歉/表达失望
...
# 命令行模式（CLI）
python main.py              # Mock 模式（无 API key 时自动进入）
...
| 变量 | 说明 | 默认值 |
|------|------|--------|
| `OPENAI_API_KEY` | OpenAI API key | 无（进入 mock 模式） |
